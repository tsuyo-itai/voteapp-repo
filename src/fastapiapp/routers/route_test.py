...
# TestModel 一覧取得
@test_router.get("/api/test", response_model=List[TestModel])
async def get_tests(request: Request):
    res = await db_get_tests()
    return res

# TestModel 1件取得
@test_router.get("/api/test/{id}", response_model=TestModel)
...
# TestModel 1件更新
@test_router.put("/api/test/{id}", response_model=TestModel)
...
# TestModel 1件削除
@test_router.delete("/api/test/{id}")
